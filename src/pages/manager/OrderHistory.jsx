    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {order.customer_name && (
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: t.text3 }}>Customer</div>
            </div>
          </div>
        )}
        ...
        <div style={{ background: t.bg3, borderRadius: 12, overflow: 'hidden', border: `1px solid ${t.border}` }}>
          <div style={{ padding: '10px 14px', borderBottom: `1px solid ${t.border}`, fontSize: 11, fontWeight: 700, color: t.text3, letterSpacing: 0.5, textTransform: 'uppercase' }}>
            Items ({items.length})
          </div>
          {items.length === 0 ? (
            <div style={{ padding: '20px', textAlign: 'center', color: t.text4, fontSize: 13 }}>No item details available</div>
          ) : (
            items.map((item, i) => (
              <div key={item.id || i} style={{
                ...
                <div>
                  ...
                  {item.discount_pct > 0 && (
                    <span style={{ fontSize: 11, color: '#F59E0B', marginLeft: 6 }}>-{item.discount_pct}%</span>
                  )}
                </div>
...
export function OrderHistory({ settings }) {
  ...
  return (
    <div style={{ padding: 'clamp(16px,2.5vw,28px)', display: 'flex', flexDirection: 'column', gap: 20, minHeight: '100%' }}>

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 900, color: t.text, letterSpacing: -0.5 }}>Order History</h1>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 10, color: '#22D3EE', fontSize: 13, fontWeight: 600, cursor: 'pointer',
            transition: 'all 0.2s',
          }}
      ...
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 20px', flexDirection: 'column', gap: 16 }}>
          <div style={{ width: 40, height: 40, border: '3px solid rgba(34,211,238,0.15)', borderTopColor: '#22D3EE', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
          <div style={{ fontSize: 14, color: t.text3, fontWeight: 600 }}>Fetching orders…</div>
        </div>
      ) : error ? (
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>⚠️</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#F87171', marginBottom: 6 }}>Failed to load orders</div>
          <div style={{ fontSize: 13, color: t.text3, marginBottom: 16 }}>{error}</div>
          <button onClick={fetchOrders} style={{ padding: '8px 20px', background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: 8, color: '#F87171', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Retry</button>
        </div>
      ) : (
        <div style={{ background: t.card || 'rgba(255,255,255,0.03)', borderRadius: 16, border: `1px solid ${t.border}`, overflow: 'hidden' }}>
                ...
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: '60px 20px', color: t.text4 }}>
                      ...
                      <div style={{ fontSize: 12, marginTop: 4, color: t.text3 }}>Try adjusting your filters</div>
                    </td>
                  </tr>
                ) : (
                  paginated.map((order, i) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px', borderBottom: `1px solid ${t.border}`, verticalAlign: 'middle' }}>
                        <div style={{ fontWeight: 700, color: '#22D3EE', fontFamily: 'monospace', fontSize: 13 }}>
                      ...
                      <td style={{ padding: '12px 16px', borderBottom: `1px solid ${t.border}`, verticalAlign: 'middle' }}>
                        {order.customer_name ? (
                          ...
                          <span style={{ color: t.text4, fontSize: 12 }}>Walk-in</span>
                        )}
                      </td>
                      {/* Date */}
                      <td style={{ padding: '12px 16px', borderBottom: `1px solid ${t.border}`, verticalAlign: 'middle', color: t.text3, whiteSpace: 'nowrap', fontSize: 12 }}>
                      ...
                      <td style={{ padding: '12px 16px', borderBottom: `1px solid ${t.border}`, verticalAlign: 'middle' }}>
                        <button
                          ...
                          style={{
                            ...
                            padding: '6px 14px', background: 'rgba(34,211,238,0.1)',
                            border: '1px solid rgba(34,211,238,0.25)', borderRadius: 8,
                            color: '#22D3EE', fontSize: 12, fontWeight: 700, cursor: 'pointer',
                            transition: 'all 0.15s',
                          }}
                          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(34,211,238,0.2)'; e.currentTarget.style.transform = 'scale(1.03)' }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'rgba(34,211,238,0.1)'; e.currentTarget.style.transform = 'scale(1)' }}
                        >
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, padding: '14px 20px', borderTop: `1px solid ${t.border}` }}>
              ...
              >← Prev</button>
              <span style={{ fontSize: 13, color: t.text3, padding: '0 8px' }}>
