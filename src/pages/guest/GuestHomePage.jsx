          ...
          <>
            ...
            <div style={{ position: 'relative', maxWidth: 1100, margin: '0 auto', padding: '0 5%', height: '100%', display: 'flex', alignItems: 'center' }}>
              <div style={{ color: '#fff', maxWidth: 560 }}>
                ...
                <div style={{ fontSize: 'clamp(26px,5vw,54px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 16, letterSpacing: -1, textShadow: '0 2px 20px rgba(0,0,0,.3)' }}>{currentBanner?.title || 'SCSTix EPOS'}</div>
                <div style={{ fontSize: 19, opacity: 0.9, marginBottom: 28, lineHeight: 1.5 }}>{currentBanner?.subtitle}</div>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                  <button onClick={() => navigate('/shop')} style={{ background: '#fff', color: currentBanner?.color || t.accent, border: 'none', borderRadius: 12, padding: '14px 28px', fontSize: 15, fontWeight: 900, cursor: 'pointer', boxShadow: '0 4px 20px rgba(0,0,0,.2)' }}>Shop Now →</button>
                  <button onClick={() => navigate('/register')} style={{ background: 'rgba(255,255,255,.15)', color: '#fff', border: '2px solid rgba(255,255,255,.5)', borderRadius: 12, padding: '14px 28px', fontSize: 15, fontWeight: 700, cursor: 'pointer', backdropFilter: 'blur(4px)' }}>Register Free</button>
                </div>
              </div>
            </div>
